...
import {
  ...
  ORGANISATIONS_FILTER_SET,
  ORGANISATIONS_PAGE_SET,
} from "../actions"
...
describe("organisations Filter container", () => {
  beforeEach(() => {
    vi.stubGlobal(
      "fetch",
      vi.fn(() =>
  ...
  it("dispatches filter set, clear, load and page set on search submit", async () => {
    ...
    await user.type(screen.getByRole("textbox"), "школа")
    ...
    expect(actions[0].payload).toEqual({ search: "школа" })
    const call = actions[2][CALL_API]
    expect(call.endpoint).toBe("organisation/?search=школа&page=1")
    expect(store.getState().filters.orgFilter).toEqual({ search: "школа" })
    expect(store.getState().pagination.orgActivePage).toBe(1)
  })

  it("does not dispatch when the submitted search is unchanged", async () => {
    const user = userEvent.setup()
    const state = makeState()
    state.filters.orgFilter = { search: "школа" }
