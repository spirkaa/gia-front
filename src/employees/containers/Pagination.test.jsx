...
import { EMPLOYEES_PAGE_SET } from "../actions"
...
describe("employees Pagination container", () => {
  beforeEach(() => {
    vi.stubGlobal(
      "fetch",
      vi.fn(() =>
  ...
  it("dispatches page set and load on page click", async () => {
    ...
    const state = makeState({
      entities: { empPage: { 1: { count: 150, results: [] } } },
    })
    ...
    expect(call.endpoint).toBe("employee/?search=&page=2")
    expect(store.getState().pagination.empActivePage).toBe(2)
  })
