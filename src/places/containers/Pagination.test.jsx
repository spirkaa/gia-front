...
import { PLACES_PAGE_SET } from "../actions"
...
describe("places Pagination container", () => {
  beforeEach(() => {
    vi.stubGlobal(
      "fetch",
      vi.fn(() =>
  ...
  it("dispatches page set and load on page click", async () => {
    ...
    const state = makeState({
      entities: { placesPage: { 1: { count: 150, results: [] } } },
    })
    const { actions, store } = renderPagination(state)
    await user.click(screen.getByText("2"))

    expect(actions.map((a) => a.type || CALL_API)).toEqual([PLACES_PAGE_SET, CALL_API])
    expect(actions[0].payload).toBe(2)
    const call = actions[1][CALL_API]
    expect(call.endpoint).toBe("place/?search=&page=2")
    expect(store.getState().pagination.placesActivePage).toBe(2)
  })
